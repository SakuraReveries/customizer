...
union() {
    ...
    rotate([180, 0, 0]) translate([coil_turns * coil_packing_factor, 0.25, coil_outer_diameter / 2]) rotate([0, 90, -90]) cylinder(d = cable_diameter, h = straight_length);
    
    translate([0, -straight_length - connector_length, -coil_outer_diameter / 2]) rotate([90, 0, 0]) cylinder(d = cable_diameter, h = straight_length);
}